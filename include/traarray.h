...
// Check if given node is left most node of a layer.
...
class TrAArray
{
 ...
 public:
  ...
  template<class I>
  int Build(const I start, const I end);         // Returns -1 if
                                                 // size != num_elements.
  unsigned long Size() {return num_elements_;}  // Returns number of
                                                // elements stored.
  unsigned long Find(const T& key);
  int Set(const unsigned long& element, const T& new_key);
  const T Sum(const unsigned long& end);
  const T Sum(const long unsigned& start, const long unsigned& end);
